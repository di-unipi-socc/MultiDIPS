...
optimumDips(Output) :-
    ...
    findall((P, Cap), globalIntent(P, smaller, Cap, _), GlobalProperties),
    optimalPlacement() %TODO:
    reverse(AllIntents, ReverseIntentList),
    findUnfulfilledIntent(ReverseIntentList, P),
    Output = (L, NewPR, NewC, NewE, P, ABW, U).

optimalPlacement(AllIntents, GlobalProperties, BestSol) :-
    findall(P, validPlacement(AllIntents, GlobalProperties, P), Sols),
    sort(2, @>=, Sols, [BestSol|_]).

validPlacement(IntentList, GlobalProperties, P) :-
    placement(IntentList, GlobalProperties, [], [], (0, 0, 0, 0, [], [], []), P).

placement([intent(StakeHolder, IID, NUsers, _)|Is], GlobProps, OldPsInfo, NewPsInfo) :-
    OldPsInfo = (_, _, _, _, OldPs, OldAllocBW, _),
    delivery(StakeHolder, IID, NUsers, OldPs, OldAllocBW, PInfo),  
    checkGlobalProperties(OldPsInfo, PInfo, GlobProps),
    mergePlacements(OldPsInfo, PInfo, TmpPsInfo),
    placement(Is, GlobProps, TmpPsInfo, NewPsInfo).
placement([], _, _, _, PsInfo, PsInfo).
...
findUnfulfilledIntent([intent(_, IID, _, _)|Is], [[]|Ps]) :- write('Cannot satisfy intent: '), write(IID), nl, nl, findUnfulfilledIntent(Is, Ps).
